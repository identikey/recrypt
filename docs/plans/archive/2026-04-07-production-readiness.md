...
- **`pre_pk` removed from `AccountRecord`:** PRE public keys are per-ciphertext / per-keyspace capability artifacts, not identity. They have been dropped from `AccountRecord`, the SQLite `accounts` schema, the `POST /accounts` request/response bodies, and the CLI registration path. The CLI `share create` command is temporarily disabled (returns an error) until the Keyspace/Grant sprint reroutes recipient PRE key discovery through a capability store.
