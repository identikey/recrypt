...
fn make_recrypt_key(encryptor: &HybridEncryptor<MockBackend>, from: &TestIdentity, to: &TestIdentity) -> String {
...
async fn test_share_create_and_fetch() {
    ...
    assert_eq!(resp.status(), 200, "get_share failed: {:?}", resp.text().await);
    ...
    // Must have bao_hash
    let bao_hash = body["bao_hash"].as_str().expect("bao_hash must be a string");
    ...
    let ciphertext_url = body["ciphertext_url"].as_str().expect("ciphertext_url must be a string");
    assert!(!ciphertext_url.is_empty());

    // Must have outboard_url
    let outboard_url = body["outboard_url"].as_str().expect("outboard_url must be a string");
    assert!(outboard_url.ends_with(".obao"), "outboard_url must end with .obao: {outboard_url}");

    // Must NOT have ciphertext field
    assert!(body.get("ciphertext").is_none(), "response must not contain raw ciphertext field");
}
...
async fn test_share_revoke() {
    ...
    assert!(
        ...
        "revoke failed with status {}: {:?}", resp.status(), resp.text().await
    );
...
async fn test_share_list() {
    ...
    assert_eq!(resp.status(), 200, "list_shares failed: {:?}", resp.text().await);

    let body: serde_json::Value = resp.json().await.unwrap();
    // Response is { outgoing: [...], incoming: [...] }
    let outgoing = body["outgoing"].as_array().expect("list_shares must return outgoing array");
    assert!(
        !outgoing.is_empty(),
    ...
    let found = outgoing.iter().any(|s| {
        s["share_id"].as_str().unwrap_or("") == share_id
    });
    assert!(found, "created share_id {share_id} not found in outgoing: {outgoing:?}");
}
...
async fn test_multi_recipient_share() {
    ...
    assert!(bob_body["wrapped_key_for_recipient"].as_str().unwrap().len() > 0);
    assert!(carol_body["wrapped_key_for_recipient"].as_str().unwrap().len() > 0);
}
...
async fn test_revoke_one_preserves_others() {
    ...
    let resp_carol = api.get_share(&carol, &share_id_carol).await;
    assert_eq!(resp_carol.status(), 200, "Carol should still access her share");
}
...
async fn test_invalid_signature_rejected() {
    ...
    let corrupted_ed_sig = {
        let mut bytes = base64::engine::general_purpose::STANDARD.decode(&ed_sig).unwrap();
        bytes[0] ^= 0xFF;
        bytes[1] ^= 0xFF;
...
async fn test_nonce_endpoint() {
    ...
    assert!(body_text.trim().len() >= 4, "nonce seems too short: {body_text:?}");
}
...
async fn test_get_account_public() {
    ...
    assert_eq!(resp.status(), 200, "get_account failed: {:?}", resp.text().await);
